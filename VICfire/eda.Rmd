...
## read ERA5 data
```{r}
era_nc <- ncdf4::nc_open(here::here("data/era5/era5.nc"))
```

```{r, eval=FALSE}
# --- print metadata into text file
{
  sink(here::here("data/era5/era_nc_metadata.txt"))
  print(era_nc)
  sink()
}
```
...
for(i in 1:length(era_var_names)){
  ...
  var_raster_brick <- raster::brick(here::here("data/era5/era5.nc"),
                                    ncdf = T,
                                    varname = var)
...
era_cluster_join_model_df <- era_cluster_join_model_df %>% 
  select(-c(cvh, lai_hv, lai_lv, cvl, slt, tvh, tvl))
...
```{r}
weather_station <- sf::st_read(here::here("data/weather_station/WEATHSTN100.shp"))
```
