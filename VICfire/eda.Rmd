...
### comparing maps of historical vs. satellite data *location
```{r}
# --- function; extract `sf` polygon; for each grid cell; to facet by `year`

extract_ignition_raster_values_training2 <- function(year){
  ...
  # • `fire_prop`: no. of points / total points (bushfire ignitions)
   ...
   ignition_rasterize_training_sp <- as(ignition_rasterize_training, "SpatialPolygonsDataFrame")
   
   ignition_rasterize_training_sf <- sf::st_as_sf(ignition_rasterize_training_sp) %>%
     rename(fire_count = layer) %>% 
...
ignition_rasterize_training_sf <- purrr::map(.x = 2000:2018,
                                             .f = extract_ignition_raster_values_training2)

# --- bind list of `sf` polygon objects into 1
ignition_rasterize_training_sf <- dplyr::bind_rows(ignition_rasterize_training_sf)
```
...
hist_map_facet_year <- tmap::tm_shape(ignition_rasterize_training_sf) +
  tmap::tm_polygons(col = "fire_count",
                    id = "id",
                    style = "cont",
                    palette = "YlOrRd",
...
```{r}
sat_map_facet_year <- tmap::tm_shape(vic_raster_crop_values$fire_count) +
