...
linearmodel <- lm(log(fire_count+1) ~ .-id -x -y, data = data_lagged, na.action= na.exclude)
summary(linearmodel)
```
...
set.seed(2021)
rand1 <- randomForest(log(fire_count+1) ~ .-year -id, data = data_lagged_omitted, importance = TRUE, mtry = 6, ntree = 500) 
rand1
```
MSE looks good?
MSE: 0.161
