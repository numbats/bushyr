...
## ERA5 data (Fire danger indices) *huge files
...
join_sf <- join_sf %>% 
  # rename x y (cell centroids)
  # rename(x = x.x,
  #        y = y.x) %>% 
  dplyr::select(-x.y, -y.y, -x.x, -y.x)
```
...
# --- write `model_df` as csv 
readr::write_csv(model_df,
                 file = here::here("data/model_df")) 
```
...
```{r}
library(tidymodels)
rpart_mod <- decision_tree(cost_complexity = 0.003) %>% 
  set_engine("rpart") %>% 
  set_mode("regression") %>% 
  translate()
...
rf_fit <- rpart_mod %>% 
  parsnip::fit(fire_count ~ .,
               data = model_df %>% dplyr::select(-id, -year, -month))

rpart.plot::rpart.plot(rf_fit$fit)
```
...
lasso_grid <- tune::tune_grid(
  ...
  resamples = fire_folds, # across 10 folds
