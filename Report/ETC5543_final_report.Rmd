...
```
    ...
    combined 
Fire ignitions are represented as spatial points on the map. There were two sources of bushfire ignitions data. The gridded Victoria map overlayed with fire ignitions in 2019-2020 bushfire season is plotted above.

Firstly, hotspot satellite data from Himawari-8 satellite (P-Tree System, 2020). These hotspot data provides a high temporal- providing data in 10-minute time intervals. It is has high spatial resolution, with the ability to monitor remote areas inaccessible by foot. This data was taken from the Japan Aerospace Exploration Agency FTP site. Himawari-8 data is a remote sensing data which are collected by remote sensors carried by the satellite. It is selected to those within the boundary of Victoria. To reduce noise, it is then filtered based on the fire power with a recommended threshold of over 100 (irradiance over 100 watts per square metre) (Williamson, 2020) From these hotspot data, fire ignitions were detected using a clustering algorithm. This allow us to detect fire ignitions in real-time. (more information available in [Patrick's github](https://github.com/TengMCing/bushfire-paper)). 

Secondly, historical First Responder's data, this can be thought of as fires reported upon sight. This data is sourced from the Victorian Department of Environment, Land, Water and Planning (DELWP). The data set provides location and causes of historical bushfire ignitions based on where the first responder saw the fire. and recorded by crews. Most of these were reports from volunteers manning fire towers erected around Victoria. (see appendix for map of fire towers around Victoria). 
