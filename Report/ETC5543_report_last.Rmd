...
The current state of the historical bushfire risk Information tab is shown in Figure \@ref(fig:app-hist-info). User interactivity includes toggling the values of the variables (max_temp, radiation, rh, et_short_crop, si10, s0_pct, daily_raain, lai_hv). These variables are chosen based on the variable importance using `LIME` package. On the left side of the app is where users can play around with the variables. The changes are based on the standard deviations of each variables for each cell id with a range of -4sd to 4sd (100%? talk more about this). 
