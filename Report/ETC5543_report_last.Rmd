...
To demonstrate, we fit a simple linear model and compute the Variance Inflation Factor (VIF). The 6 variables with the highest VIF is shown in Table \@ref(tab:vif-tab). A general rule of thumb is that if VIF $> 10$ then multicollinearity is high @andreaperlato. Lasso's objective function provides unstable solutions in prescence of collinear features or features with very similar information. @schreiber2018regulation 
...
## Final Model
The final model is fitted on all of the data, including the test set, using the `ranger` package due to the much faster computation compared to the `randomforest` package. This model has a 0.42 R-squared and 1.92 of MSE. Summary statistics of the model can be seen on Table \@ref(tab:final-model-summary). 

```{r final-model-summary}
model <- readRDS(here::here("VICfire/model/rfmodel_final.rds"))
final_data <- read.csv(here::here("VICfire/data/model_df2.csv")) %>% na.omit()

predictions_final <- model$predictions
predicted_data_final <-  final_data %>% 
  bind_cols(predictions = predictions_final)
kable(model_summary(truth = final_data$fire_count,
           estimate = predictions_final),
      caption = "Summary statistics of the final random forest model.")
```


# Shiny Web application (User-Interface)

The outputs of data processing and bushfire predictions are put together in an app. This app is an extension to an previously made `RShiny` app (publicly available at [Monash University's Department of Econometrics and Business Statistics website](https://ebsmonash.shinyapps.io/VICfire/). The previous app allow users to visualise bushfires in Victoria at a period of time of their choosing. Outputs are based on the clustering results. Also, a model predicts the cause of bushfire; given a bushfire ignition. Our complementary goal (as per Research grant) is to develop risk model to visualise and monitor potential fire ignitions and track fires from satellite hotspots, in real time, for 2021-2022 Victorian bushfire season.   
...
The current project builds on the "Fire Risk" Tab. On top of the historical bushfire information and bushfire `cause` prediction, this complementary addition serves to inform users with historical information risk and up-to-date predictions around bushfire risk. We maintained a similar design for the structure of the app - keeping separate tabs for historical information and predictions. This is to cater for different users who might be using the app. Fire personnels might be interested in the weather and landscape variables creating the conducive fire environment. Plots and data are made downloadable. The github link is also provided in the app to allow users to download the model object and play around with it. While users would be interested in bushfire predictions on the go while planning a trip. User can choose the bushfire seasons to overlay historical ignition points.  

```{r bushfire-app, fig.cap="Bushfire Risk Prediction tab"}
knitr::include_graphics(here::here("VICfire/data/report/bushfire-app.png"))
```

The final state of the bushfire risk prediction tab is shown in Figure \@ref(fig:bushfire-app). The main panel of the app shows a map of Victoria in a 20x20 grid cells with bushfire risk predictions for each of the cells in terms of proportions, calculated based on the baseline total, which is the total fires in the monthly baseline average. The random forest model is incorporated to predict the risk of bushfire based on weather variables. Users will be able to toggle the various variables and make forecasts under different scenarios.

User interactivity includes toggling the values of the variables (daily rain, evapotranspiration rate, maximum temperature, radiation, relative humidity, wind speed, and surface soil moisture). These variables are chosen based on the variable importance using `LIME` package. On the left side of the app is where users can play around with the variables. The changes are based on the standard deviations of each variables for each cell id with a range of -4sd to 4sd, which means that -100% is referring to the values being deducted by 4 standard deviations, while 100% means that the values is being increased by 4 standard deviations. The slider input is limited to values which are in the range of the variables. When users change the values, the updated predictions would be visualised on the map after the apply button is clicked. A choice of months is presented on the app to allow users to choose which months they want to visualise. After playing around with the variables, users can reset the variables to the initial average values by clicking the `reset` button. 

Upon clicking a cell, the means and standard deviations of each of the variables for each cell id are shown on the right side of the slider input. Other than that, plots of the historical values of the weather variables will be provided on the right panel, while a data table presenting the predictions will be shown on the bottom side of the app. These plots illustrate the historical values of the variables, presented with the green line. When users change the values, the pink line will change to represent the updated values. A label presenting the predictions will pop up as shown in Figure \@ref(fig:clicked-cell). [NEED TO UPDATE THIS]

```{r clicked-cell, fig.cap="When users click on a specific cell, information related to the cell will pop out."}
knitr::include_graphics(here::here("VICfire/data/report/clicked-cell.png"))
```

This prediction app will be combined with the historical bushfire app to provide a comprehensive information for users.

# Final insights
Based on the risk predictions model, different months have different risks of bushfire with March being the most risky month, followed by January and December. October is the least risky month. Another insight is that different months have different risky locations. These following locations are deemed to be generally risky of bushfires: 
- Inglewood  
- Horsham  
- Edenhope  
- Hamilton  
...
- Warburton   
- Macalister Gorge Natural Features and Scenic Reserve  
- Lake Gnarpurt  
- Katamatite  
...
The final model is fitted on all of the data, including the test set. This results to a model with 0.42 R-squared and 1.92 MSE. Even though random forest does not provide a perfect prediction, it provides a pretty accurate predictions, especially if looking at the relative counts in proportion-wise. The model predicts cell which have higher observed fires to have higher predicted fires in comparison to other cells. 

Even though more than 80% of fires occur by lightning, another limitation of our model is it uses variables which represent weather conditions, which might not accurately account for accident-based fires.
Other variables like distance to road is not suitable to be implemented in the model because our work is based on gridded cells and we do not have one specific ignition point. Therefore, we could not really calculate the distances of each grid cells. An alternative would be to treat distance to road as a dummy variable with a value of 1 if the cell consists of road and 0 otherwise. However, most cells contain roads and it is unlikely to be a useful predictor. Other variables that might be useful for predicting the risk of non-lightning caused fires could be further explored and factored in the model. Additionally, time of day could be added so that users could choose which time of day they want to present in the app, because different time of day might result to different bushfire risk. 
